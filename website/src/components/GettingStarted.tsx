  ...
  return (
    <Card title="What to Install">
      <p class="text-[13px] text-text-dim leading-[1.7] mb-3">
        Everything starts with Xcode — Apple's IDE that includes the Swift compiler, simulators, and
        Interface Builder.
      </p>
      <Step n={1}>
        <span>
          Install{" "}
          <ExtLink href="https://apps.apple.com/us/app/xcode/id497799835">
      ...
      <Step n={2}>
        <span>Open Xcode once to install additional components</span>
      </Step>
      <Step n={3}>
        <span>
...
function EmptyFolderToApp() {
  return (
    <Card title="From Empty Folder to Running App">
      ...
      <Step n={2}>
        <span>
          Choose <strong class="text-text">macOS {"\u2192"} App</strong>
        </span>
      </Step>
      <Step n={3}>
        <span>
          Pick a name, set language to <strong class="text-text">Swift</strong>, interface to{" "}
          <strong class="text-text">SwiftUI</strong>
        </span>
      </Step>
      <Step n={4}>
        <span>
          Click <strong class="text-text">Run</strong> (or <Code>{"\u2318"}R</Code>) {"\u2014"} you
          have a working app
        </span>
      </Step>
    </Card>
...
function WhatIsAppDelegate() {
  return (
    <Card title="What is AppDelegate?">
      <p class="text-[13px] text-text-dim leading-[1.7] mb-3">
        The <Code>AppDelegate</Code> is your app's entry point on macOS. The system calls it when
        your app launches, goes to background, or terminates.
      </p>
      <div class="flex items-center gap-2 flex-wrap my-4">
        <span class="px-3.5 py-1.5 rounded-md font-mono text-[11px] bg-sage-glow border border-sage-dim text-sage">
          macOS
        </span>
        <span class="text-text-muted text-sm">{"\u2192"}</span>
        <span class="px-3.5 py-1.5 rounded-md font-mono text-[11px] bg-bg-code border border-border text-orange">
          AppDelegate
        </span>
        <span class="text-text-muted text-sm">{"\u2192"}</span>
        <span class="px-3.5 py-1.5 rounded-md font-mono text-[11px] bg-bg-code border border-border text-orange">
      ...
      <p class="text-[13px] text-text-dim leading-[1.7]">
        Think of it as the "main()" of your macOS app {"\u2014"} the first code that runs when your
        app starts.
      </p>
    </Card>
  );
...
function SwiftUIVsAppDelegate() {
  return (
    <Card title="SwiftUI App vs AppDelegate">
      <p class="text-[13px] text-text-dim leading-[1.7] mb-3">
        Modern SwiftUI apps use <Code>@main struct MyApp: App</Code> as the entry point. But Flowbar
        uses <strong class="text-text">both</strong> {"\u2014"} an <Code>AppDelegate</Code> plus
        SwiftUI views.
      </p>
      <p class="text-[13px] text-text-dim leading-[1.7]">
        <strong class="text-text">Why?</strong> Menu bar apps need <Code>NSStatusBar</Code>, popover
        management, and global keyboard shortcuts {"\u2014"} all AppKit APIs that require an{" "}
        <Code>AppDelegate</Code>. SwiftUI handles the UI inside the popover.
      </p>
    </Card>
  );
}
  ...
  return (
    <Card title="What's in a Swift Project?">
      ...
      <ul class="space-y-2 text-[13px] text-text-dim leading-[1.7]">
        <li>
          <Code>.xcodeproj</Code> {"\u2014"} The project file Xcode uses to track build settings,
          targets, and file references
        </li>
        <li>
          <Code>.swift</Code> {"\u2014"} Your source files. Each file can contain structs, classes,
          enums, and functions
        </li>
        <li>
          <Code>Info.plist</Code> {"\u2014"} App metadata: name, version, permissions, and
          capabilities
        </li>
        <li>
          <Code>Assets.xcassets</Code> {"\u2014"} App icons, images, and color assets organized in a
          catalog
        </li>
      </ul>
    </Card>
