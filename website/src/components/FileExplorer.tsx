...
export function FileExplorer({ initialFile = "App/FlowbarApp.swift", selectedFile }: FileExplorerProps) {
    const [activeFile, setActiveFile] = useState(initialFile);
    const [popup, setPopup] = useState<{ title: string; body: string; x: number; y: number } | null>(null);
    useEffect(() => {
    ...
    function renderCode(path: string) {
        ...
        for (const lineNum of Object.keys(fileAnnotations).map(Number).sort((a, b) => a - b)) {
            annotationCounter++;
    ...
    return (
        <section class="py-[100px] px-12 max-w-[1280px] mx-auto" id="explorer">
            ...
            <p class="text-[17px] text-text-dim max-w-[640px] leading-[1.8]">
                Browse the complete source code. Numbered markers highlight key Swift
                concepts — hover or click them for explanations.
            </p>
